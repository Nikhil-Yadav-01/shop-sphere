...
SERVICE_PORT="${SERVICE_PORT:-${SERVER_PORT:-8091}}"
BASE_PATH="${BASE_PATH:-/api/v1/media}"
...
BASE_URL="${BASE_URL:-http://${SERVICE_IP}:${SERVICE_PORT}${BASE_PATH}}"
...
TMP_ROOT="${TMPDIR:-/tmp}"
TEST_DIR="$(mktemp -d "${TMP_ROOT%/}/media_tests.XXXXXX" 2>/dev/null || echo "${TMP_ROOT%/}/media_tests")"
mkdir -p "$TEST_DIR"
...
url="$BASE_URL/health"
resp_and_code=$(${CURL_BIN} -s -w "\n%{http_code}" "$url")
body=$(echo "$resp_and_code" | sed '$d')
code=$(echo "$resp_and_code" | tail -n1)
if [[ "$code" == "000" ]]; then
    fail "Cannot connect to media service (HTTP 000). Is the service running and is the port correct?" "$body"
    print_debug "$url" "$code" "$body"
    echo -e "\n${RED}Aborting test suite because the target service is unreachable.${NC}\n"
    rm -rf "$TEST_DIR"
    exit 2
fi
if [[ "$code" == "200" ]]; then
...
for i in 1 2 3 4 5; do
    ( ${CURL_BIN} -s -o /dev/null -w "%{http_code}" -F "file=@$TEST_DIR/test.png" -F "entityType=concurrent" -F "entityId=1234" "$BASE_URL/upload" ) &
    pids+=($!)
done
