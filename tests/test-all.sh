...
echo ""
echo "17. Batch Service - Health (8094):"
curl -s http://$IP:8094/actuator/health | grep -o '"status":"[^"]*"' || echo "FAILED"

echo "18. Batch Service - Get Failed Jobs (8094):"
curl -s "http://$IP:8094/api/v1/batch/jobs/failed" | grep -o '"success":[^,]*' || echo "FAILED"
echo ""
echo "19. API Gateway - Batch Route (8080):"
curl -s "http://$IP:8080/batch/api/v1/batch/jobs/failed" | grep -o '"success":[^,]*' || echo "FAILED"
echo ""
echo "20. Cart Service - Health (8085):"
curl -s http://$IP:8085/actuator/health | grep -o '"status":"[^"]*"' || echo "FAILED"
echo ""
echo "21. Cart Service - Get Cart (8085):"
curl -s -H "X-User-Id: testuser" http://$IP:8085/api/v1/cart | grep -o '"userId":"[^"]*"' || echo "FAILED"
echo ""
echo "22. Cart Service - Add Item (8085):"
curl -s -X POST -H "X-User-Id: testuser" -H "Content-Type: application/json" -d '{"productId":"prod1","quantity":1}' http://$IP:8085/api/v1/cart/items | grep -o '"totalItems":[0-9]*' || echo "FAILED"
echo ""
echo "23. API Gateway - Cart Route (8080):"
curl -s -H "X-User-Id: testuser2" http://$IP:8080/cart/api/v1/cart | grep -o '"userId":"[^"]*"' || echo "FAILED"
echo ""
echo "=== Test Complete ===" 